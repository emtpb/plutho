...
alpha_M = 1.267e5
alpha_K = 6.259e-10
c = np.array([[1.19e11, 0.83e11,       0, 0.84e11],
              [0.83e11, 1.17e11,       0,       0],
              [      0,       0, 0.21e11,       0],
              [      0,       0,       0, 1.19e11]])
e = np.array([[0, 0, 12.09, 0],
              [-6.03, 15.49, 0, -6.03]])
...
q = np.zeros(N)
u = np.zeros((3*number_nodes, N))
z = np.zeros((3*number_nodes, N))
a = np.zeros((3*number_nodes, N))
...
def integral_KDeG(jac_inv, points):
    def func(x):
        return (np.dot(np.dot(gradient(x, jac_inv, points).T, e),
                       b_op_global(x, jac_inv, points)) *
                transform_coordinates(points, x)[0]) # Should it not be B.T*e.T*B_tilde
    integral = quadratic_quadrature(func)
    return integral
...
def Dirichlet(matrix, idx_list, value_list):
    f = np.zeros(3*number_nodes)
    for n, idx in enumerate(idx_list):
        matrix[idx, :] = 0
        matrix[idx, idx] = 1
        # Should it be also matrix[:, idx] = 0?
        f[idx] = value_list[n]
    print(f)
    return matrix, f
...
Dirichlet_idx = np.hstack((np.unique(np.array(sum(boundaries['"Symaxis"'], []))),
                           np.unique(np.array(sum(boundaries['"Electrode"'], []))) + 2 * number_nodes,
                           np.unique(np.array(sum(boundaries['"Ground"'], []))) + 2 * number_nodes))
integral_idx = boundaries['"Electrode"']
...
for t_idx, t in enumerate(t[:-1]):
    print(t_idx)
    q_idx = 0
    Dirichlet_value = np.concatenate((np.zeros(np.size(np.unique(np.array(sum(boundaries['"Symaxis"'], []))))),
                                pot_excite[t_idx]*np.ones(np.size(np.unique(np.array(sum(boundaries['"Electrode"'], []))))),
                                0*np.ones(np.size(np.unique(np.array(sum(boundaries['"Ground"'], [])))))))
    # predictor
    u_tilde = u[:, t_idx] + delta_t*z[:, t_idx] + (1-beta)*delta_t**2/2*a[:, t_idx]
    z_tilde = z[:, t_idx] + delta_t*(1-gamma)*a[:, t_idx]
    # solve
    matrix_dir, f_ex = Dirichlet(matrix_lhs, Dirichlet_idx, Dirichlet_value)
    matrix_dir = matrix_dir.tocsr()
    rhs = f_ex-1*K*u_tilde - C*z_tilde
    ...
    q[t_idx+1] = q_idx
print("Compuetation time:", time.time()-tic)

print("Charge per time:", q)
plt.plot(range(N), pot_excite, label="Excitation")
plt.plot(range(N), q, label="q")
plt.legend()
plt.grid()
plt.xlabel("Time t")
plt.show()
