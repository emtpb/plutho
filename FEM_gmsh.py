...
q_excite = 0*np.ones(number_nodes)
...
Dirichlet_value = np.hstack((np.zeros(np.size(np.unique(np.array(sum(boundaries['"axis"'], []))))),
                             0,
                             1*np.ones(np.size(np.unique(np.array(sum(boundaries['"electrodes12"'], []))))),
                             0*np.ones(np.size(np.unique(np.array(sum(boundaries['"electrode3"'], [])))))))
...
K = K.tocsr()
K_mech = K_mech.tocsr()
matrix_M = matrix_M.tocsr()


freq = np.arange(1, 1500) * 6250
imp = np.ones(np.size(freq))

for f_idx, f in enumerate(freq):
    print(f_idx)
    q = 0
    matrix = (K - (2*np.pi*f)**2*matrix_M + 1j*2*np.pi*f*(alpha_M*matrix_M + alpha_K*K_mech))
    f_ex_n = f_ex*(1 - (2*np.pi*f)**2 + 1j*2*np.pi*f*(alpha_M+alpha_K))
    u = slin.spsolve(matrix, f_ex_n)
    ...
    for idx in (integral_idx): # over all boundary elements in electrode
        ...
        ur_K = [u[i] for i in element]
        uz_K = [u[i+number_nodes] for i in element]
        u_K = ur_K + uz_K
        phi_K = [u[i+2*number_nodes] for i in element]
        ...
        q += np.sum(-1*int_eps + int_e)

    imp[f_idx] = abs(1/(1j*2*np.pi*f*q))
print(time.time()-tic)

plt.figure(3)
plt.semilogy(freq, imp)
#
points_x = [i[0] for i in points]
points_y = [i[1] for i in points]
plt.figure()
plt.scatter(points_x, points_y, c=abs(u[2*number_nodes:3*number_nodes]))
